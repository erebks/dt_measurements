...
BITS = 4
WATERMARK_SHIFT = 0
SPREADING_SEED = int("BEEF", 16)
HIST_BINS = 200
SUBPLOT_SIZE = [4,3]
SUPTITLE = "4 bit encoding with random values and spread spectrum"
...
def analyze(measurements):
    return helper.readMessages_nBit_ss(measurements, NOMINAL_S, TOLERANCE_S, PHASE_DELTA_S, BITS, PRINT_MATCHES, SPREADING_SEED, watermarkShift=WATERMARK_SHIFT)
...
def plot():
    ...
    # Only TS at 0s
    gw_ts = np.array(list(ele["gw_timestamp_delta_despread"] for ele in msgs), float)
    gw_ts = (gw_ts - NOMINAL_S) * 1000

    axs[2][1].hist(gw_ts, bins=HIST_BINS, color='r')
    axs[2][1].set_title("Histogram GW timestamps despreaded")
